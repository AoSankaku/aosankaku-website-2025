...
誰でも簡単にできる方法で書き直したので、こちらをご覧ください。
...
この記事の内容は古い＆めちゃムズです。そのため、残りの文章は、苦しみたい変態の方々のみお読みいただくことをおすすめします。ただし、VysorはAndroidのやり方よりも遅延が（多分）少ないので、音声共有が不要ならVysorだけ入れてみてください。
...
> [!TIP] 貼り付けのやり方（2026/08/14追記）
> `Ctrl+Shift+V`または右クリックメニューから貼り付けられるっぽいです。

```bash
pacman -S mingw-w64-ucrt-x86_64-cmake mingw-w64-ucrt-x86_64-gcc
```

1. （オプション）このままだと、UxPlayのフォルダがどこにできるかわからない（おそらく`C:Users`直下にできます）ので、`cd`（現在のフォルダ移動）したい人は適当に`mkdir`したり`cd`してください。
2. `pacman -S git`と打って実行します。
3. `git clone https://github.com/FDH2/UxPlay`と打って実行します。
4.  `cd UxPlay`と打って実行します。もしもエラーになったら、`ls`を実行してそこにあるフォルダを確認し、フォルダ名を見つけて`cd`してください。
5.  以下のコマンドを1行で打ちます。またコピペ不可です。がんばってください。
